...
namespace libAstroGrep
{
	...
	public class Grep
	{
		...
		/// </history>
		private static readonly List<string> validTexts = new List<string> { " ", "<", ">", "$", "+", "*", "[", "]", "{", "}", "(", ")", ".", "?", "!", ",", ":", ";", "-", "\\", "/", "'", "\"", Environment.NewLine, "\r\n", "\r", "\n", "=", "\t" };
		...
		/// <history>
		/// [Curtis_Beard]      02/28/2020	Created
		/// </history>
		public static Regex BuildSearchRegEx(ISearchSpec searchSpec)
		{
			Regex regEx = null;

			// create regex outside of loop
			if (searchSpec.UseRegularExpressions || searchSpec.UseWholeWordMatching)
			{
				var options = searchSpec.UseCaseSensitivity ? RegexOptions.None : RegexOptions.IgnoreCase | RegexOptions.Compiled;
				var boundary = searchSpec.UseWholeWordMatching ? "\\b" : string.Empty;

				var search = searchSpec.SearchText;
		...
		public void Execute()
		{
			...
			else
			{
				if (string.IsNullOrEmpty(SearchSpec.FileFilter))
				{
					...
					{
						Execute(new DirectoryInfo(dir), null, null, isStartDirectory: true);
					}
				}
				else
				{
		...
		/// <history>
		/// [Curtis_Beard]	   03/07/2012	ADD: 3131609, exclusions
		/// [Curtis_Beard]      02/28/2020  Update to null check/invoke .net 4.5
		/// </history>
		...
		{
			SearchingFileByPlugin?.Invoke(file, pluginName);
		}
		...
		/// [Curtis_Beard]	   03/07/2012	ADD: 3131609, exclusions
		/// [Curtis_Beard]	   09/29/2016	CHG: 24/115, use one interface for search in prep for saving to file
		/// [Curtis_Beard]	   09/09/2019	CHG: don't look at temp based files that start with ~$
		/// </history>
		private static bool ShouldFilterOut(FileInfo file, ISearchSpec searchSpec, out FilterItem filterItem, out string filterValue)
		{
			...
			// always filter out temp files
			if (file.Name.StartsWith("~$"))
			{
				filterItem = new FilterItem(new FilterType(FilterType.Categories.File, FilterType.SubCategories.Name), "~$", FilterType.ValueOptions.StartsWith, true);
				filterValue = file.Name;
				return true;
			}
					...
					else
					{
						...
						if (_posInStr > -1)
						{
							...
							if (SearchSpec.ContextLines > 0 && _lastHit <= 0)
							{
								...
								_contextIndex -= tempContextLines;
								if (_contextIndex < 0)
								{
