...
namespace libAstroGrep
{
	...
	public class FilterItem
	{
		...
		/// <summary>
		/// Copy the current item values to a new instance of a FilterItem.
		/// </summary>
		/// <returns></returns>
		/// <history>
		/// [Curtis_Beard]	   04/09/2019	FIX: 114, allow copy of item to a new instance
		/// </history>
		public FilterItem Copy()
		{
			return new FilterItem(FilterType, Value, ValueOption, ValueIgnoreCase, ValueSizeOption, Enabled);
		}
		...
		/// <history>
		/// [Curtis_Beard]	   10/31/2014	ADD: exclusions update
		/// [Curtis_Beard]	   03/05/2020	CHG: 144, Add ability to exclude more than one extension at once
		/// </history>
			...
			if (Enabled && FilterType.Category == FilterType.Categories.File)
			{
				switch (FilterType.SubCategory)
				{
					case FilterType.SubCategories.Extension:
						...
						foreach (string temp in Value.ToLower().Split(new char[] { ',', ';' }, StringSplitOptions.RemoveEmptyEntries))
						{
							string t = temp;
							if (file.Extension.StartsWith(".") && !t.StartsWith("."))
							{
								t = string.Format(".{0}", t);
							}
							value = t;

							if (file.Extension.ToLower().Equals(t))
							{
								return true;
							}
						}
		...
		/// [Curtis_Beard]	   06/28/2022	CHG: 123, Strings should allow not contains
		/// </history>
		private bool CheckStringAgainstOption(string checkValue)
		{
			if (Enabled && FilterType.ValueType == FilterType.ValueTypes.String)
			{
				switch (ValueOption)
				{
					...
					case FilterType.ValueOptions.NotContains:
						return checkValue.IndexOf(Value, ValueIgnoreCase ? StringComparison.InvariantCultureIgnoreCase : StringComparison.InvariantCulture) == -1;
