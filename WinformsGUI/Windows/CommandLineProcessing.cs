...
namespace AstroGrep.Windows
{
	...
	/// [/clb="value"]                  - Number of context lines before
	/// [/cla="value"]                  - Number of context lines after
	...
	public class CommandLineProcessing
	{
		...
		private static void InitializeArgs(ref CommandLineArguments args)
		{
			...
			args.ContextLines = -1;
			args.ContextLinesBefore = -1;
			args.ContextLinesAfter = -1;
		...
		private static void ProcessFlags(Arguments myArgs, ref CommandLineArguments args)
		{
			...
			if (myArgs["clb"] != null)
			{
				try
				{
					int num = int.Parse(myArgs["clb"]);

					if (num >= 0 && num <= Constants.MAX_CONTEXT_LINES)
						args.ContextLinesBefore = num;
				}
				catch { }
			}
			...
			if (myArgs["cla"] != null)
			{
				try
				{
					int num = int.Parse(myArgs["cla"]);

					if (num >= 0 && num <= Constants.MAX_CONTEXT_LINES)
						args.ContextLinesAfter = num;
				}
				catch { }
			}
			...
			if (myArgs["dmf"] != null)
			{
				...
				if (values.Length == 2)
				{
					...
					if (DateTime.TryParse(values[1], out DateTime value) && valueOption != libAstroGrep.FilterType.ValueOptions.None)
					{
			...
			if (myArgs["minfsize"] != null)
			{
				...
				{
					libAstroGrep.FilterType.ValueOptions valueOption = GetValueOptionFromOperator(values[0]);
					if (long.TryParse(values[1], out long value) && valueOption != libAstroGrep.FilterType.ValueOptions.None)
					{
			...
			{
				if (int.TryParse(myArgs["minfc"], out int value))
				{
