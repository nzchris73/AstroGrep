...
namespace AstroGrep.Windows.Forms
{
	...
	public partial class frmCheckForUpdateTemp : BaseForm
	{
		...
		/// <history>
		/// [Curtis_Beard]	   11/26/2014	Initial: partial update to update checking
		/// [Curtis_Beard]	   02/23/2023	CHG: use latest Tls so that redirect to https works
		/// </history>
		...
		private void CheckForUpdate()
		{
			try
			{
				...
				using (WebResponse webRes = webReq.GetResponse())
				{
					...
					Common.Logging.LogClient.Instance.Logger.Info($"Successfully checked for new version, retrieved version {newestVersion.ToString(3)}, current version: {currentVersion.ToString(3)}");
				}
			}
			catch (Exception ex)
			{
				Common.Logging.LogClient.Instance.Logger.Warn(ex, $"Unable to check for update: {ex.Message}");
